...
server {
        ...
        location ~ (/api/|/swagger-ui.html|/swagger-ui/|/v3/api-docs) {
...
# 실행 중인 도커 컴포즈 확인
EXIST_A=$(sudo docker-compose -p app-api-a -f docker-compose.a.yml ps | grep Up)
...
echo "app-api-${START_CONTAINER} up"

# 실행해야하는 컨테이너 docker-compose로 실행. -p는 docker-compose 프로젝트에 이름을 부여
# -f는 docker-compose파일 경로를 지정
sudo docker-compose -p app-api-${START_CONTAINER} -f docker-compose.${START_CONTAINER}.yml up -d --build || res=$?

if [ "${res}" = 1 ]; then
  echo "failed, app-api-${START_CONTAINER} up"
  exit 1
fi
...
sudo docker-compose -p app-api-${TERMINATE_CONTAINER} -f docker-compose.${TERMINATE_CONTAINER}.yml down || res=$?
if [ "${res}" = 1 ]; then
  echo "failed, app-api-${TERMINATE_CONTAINER} down"
  exit 1
fi

echo "success, app-api-${TERMINATE_CONTAINER} down"
