...
| 조합 | 설명 | 예시 |
|------|------|------|
| 동기 + 블로킹 | 결과 기다림 + 제어권 없음 | 일반적인 함수 호출 |
| 비동기 + 논블로킹 | 결과 안 기다림 + 제어권 있음 | `async/await`, 콜백 |
| 동기 + 논블로킹 | 결과 기다림 + 주기적 확인 | polling 방식 |
| 비동기 + 블로킹 | 드묾 (비효율적) | 잘못된 설계 |

실무에서는 **동기+블로킹** 또는 **비동기+논블로킹** 조합이 대부분입니다.
